...
open class EZSwipeController: UIViewController {
    ...
    public required init?(coder aDecoder: NSCoder) {
        ...
        navBar = NavigationBar(coder: aDecoder)
        setupView()
    }
        ...
        for index in 0..<stackVC.count {
            ...
            if let nav = navigationBar {
                if navigationBarShouldBeOnBottom {
                    ...
                    nav.frame = CGRect(x: 0, y: UIApplication.shared.statusBarFrame.height, width: Constants.ScreenWidth, height: Constants.navigationBarHeight)
                }
